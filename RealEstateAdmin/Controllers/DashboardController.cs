    ...
    public class DashboardController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            if (isAdmin)
            {
                ...
                var totalRevenue = await _context.Sales.SumAsync(s => (decimal?)s.Prix) ?? 0;
                var monthStart = new DateTime(DateTime.Now.Year, DateTime.Now.Month, 1);
                var salesThisMonth = await _context.Sales.CountAsync(s => s.CreatedAt >= monthStart);

                var topSeller = await _context.Sales
                ...
                if (topSeller != null)
                {
                    var user = await _userManager.FindByIdAsync(topSeller.SellerId);
                    topSellerName = user?.Nom ?? user?.UserName ?? "-";
                    ViewBag.TopSellerCount = topSeller.Count;
                }

                ViewBag.TotalSales = totalSales;
                ViewBag.TotalRevenue = $"{FormatMoney(totalRevenue)} DT";
                ViewBag.SalesThisMonth = salesThisMonth;
                ViewBag.TopSellerName = topSellerName;
            }
            else if (currentUser != null)
            {
                var myPurchases = await _context.Sales.CountAsync(s => s.BuyerId == currentUser.Id);
                var mySales = await _context.Sales.CountAsync(s => s.SellerId == currentUser.Id);
                var mySpend = await _context.Sales.SumAsync(s => s.BuyerId == currentUser.Id ? (decimal?)s.Prix : 0) ?? 0;

                ViewBag.MyPurchases = myPurchases;
                ViewBag.MySales = mySales;
                ViewBag.MySpend = $"{FormatMoney(mySpend)} DT";

                var myLogs = await _context.AuditLogs
                    .Where(l => l.UserId == currentUser.Id)
                    .OrderByDescending(l => l.CreatedAt)
                    .Take(5)
                    .ToListAsync();
                ViewBag.MyLogs = myLogs;
            }
